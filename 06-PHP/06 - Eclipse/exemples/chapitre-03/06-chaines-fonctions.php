...
<html xmlns="http://www.w3.org/1999/xhtml" lang="fr">
  ...
  <body>
    ...
    echo 'Mise en forme d\'un nombre (utilisation de *) : ', 
         sprintf('%0*.*f',1,3,1/3),'<br />'; 
    echo 'Options de remplissage (utilisation de *) :<br />'; 
    echo '<code>'; // police non proportionnelle 
    printf("%'.-*s<br />",10,'Total'); // printf direct 
    echo '</code >'; 
    ?>
    ...
    if (! $position) { // mauvais test : ===
      echo "'contact' est introuvable dans $mail (<b>faux !!!</b>)<br />";
    } else {
    ...
    <h1>str_contains() - str_starts_with - str_ends_with</h1>
    <?php
    $mail = 'contact@olivier-heurtel.fr';
    $x = 'olivier';
    echo "$mail contient '$x' : ",str_contains($mail,$x)?'Oui':'Non','<br />';
    $x = 'Olivier';
    echo "$mail contient '$x' : ",str_contains($mail,$x)?'Oui':'Non (casse)','<br />';
    $x = 'contact';
    echo "$mail commence par '$x' : ",str_starts_with($mail,$x)?'Oui':'Non','<br />';
    $x = '.com';
    echo "$mail se termine par '$x' : ",str_ends_with($mail,$x)?'Oui':'Non','<br />';
    ?>
